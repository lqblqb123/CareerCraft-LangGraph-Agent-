...
所有 Agent 通过一个 `TypedDict` 共享数据，12 个字段分属 4 个阶段。核心原则：**每个字段只有一个写入者**——避免两个节点抢同一个字段的并发冲突。
...
LangGraph 的 `StateGraph` 支持运行时条件分支——不是固定流水线，而是根据 Agent 判断动态决定下一步：

- Agent 觉得信息不够 → 暂停，问你问题
- 追问满 3 轮 → 强制推进，不管 Agent 怎么想
- 审查完 → 直接进入下一阶段（不回头修正）
...
**硬数据保护：** Prompt 明确列举姓名、日期、公司名、学校名、电话号码、邮箱为"绝对禁止修改"的字段。连日期格式都不能变（`2019.9` 不能写成 `2019年9月`）。
...
LangGraph interrupt() 抛出异常
    ↓
Checkpointer（SqliteSaver）自动保存当前状态到 data/checkpoints.db
    ↓
CLI 捕获中断事件，展示问题给用户
    ↓
用户输入回答
...
| 防重复 | 历史问题传给 Agent | nodes.py 累加 |

### 中断恢复

SqliteSaver 在每步执行后将状态写入 SQLite。关了程序、换了电脑，`/load <id>` + `/resume` 从中断处继续——不用重新上传简历。
...
**设计考虑：** 不包含"量化分"——量化程度影响简历可信度，但不应该影响"这人适不适合这个岗位"的判断。量化是优化师该做的事，不是评分的标准。
...
| 两个计数器 | question_rounds ≠ iteration | 一个变量承载两种语义必炸 |
| 状态每字段单写者 | 12 字段各自一个源头 | 避免并发冲突 |
...
| 层 | 技术 |
|----|------|
| Agent 编排 | LangGraph（StateGraph + Conditional Edges + Checkpointer） |
...
| 持久化 | SQLite（Session 快照 + SqliteSaver 中断恢复） |
| 导出 | Jinja2 模板 + fpdf2（Markdown + PDF 双格式） |
| 简历解析 | pdfplumber（支持中文 PDF） |
| 评测 | 20 样本数据集 + 边界值容差 |
| 代码质量 | Ruff + Black + MyPy + Pytest（10 个单元测试） |
...
│   │   ├── state.py          #   AgentState — 12 字段中央状态
│   │   ├── nodes.py          #   节点函数 — 每个 Agent 怎么被调用
│   │   ├── router.py         #   条件路由 — route_after_xxx
...
│
├── tests/
│   ├── test_workflow.py      # 工作流 + 路由 + 导出单元测试（10 个）
