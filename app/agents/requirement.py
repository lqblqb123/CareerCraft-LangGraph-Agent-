...
class CareerAdvisorResult(BaseModel):
    ...
    match_score: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="与最优岗位的匹配度，按公式(技能×0.40+经验×0.35+行业×0.15+学历软技×0.10)/100",
    )
...
## 匹配度计算公式（必须严格按此公式计算 match_score）

match_score = 技能分 × 0.40 + 经验分 × 0.35 + 行业分 × 0.15 + 学历软技分 × 0.10

各子项评分标准（每项 0-100 分，根据以下标准打分）：

| 子项 | 权重 | 100 分标准 | 60 分标准 | 0 分标准 |
|------|------|-----------|----------|---------|
| 技能分 | 40% | JD 要求的技能简历全有 | 缺 2-3 个 JD 核心技能 | 一个 JD 核心技能都不匹配 |
| 经验分 | 35% | 年限达标 + 项目高度相关 | 年限差 1-2 年或项目弱相关 | 年限差 3 年以上且项目无关 |
| 行业分 | 15% | 目标行业同领域经验 | 相关但不完全对口 | 完全跨行业 |
| 学历软技分 | 10% | 学历达标 + 有团队/管理/分享经历 | 学历达标但没有软技能 | 学历不达标 |

**计算流程**：
1. 根据上述标准为每项打 0-100 分
2. 按权重计算加权总分
3. match_score = 加权总分 / 100（即 0.0-1.0 的小数）
4. 在 candidate_profile 中必须写明各项得分和计算过程，例如："技能 85/100, 经验 70/100, 行业 60/100, 学历软技 80/100 → (85×0.40 + 70×0.35 + 60×0.15 + 80×0.10) / 100 = 0.75"
